...
type UserActiveBusinessContext struct {
	...
	UserID     uuid.UUID         `gorm:"type:uuid;not null;index:idx_user_active_business_user_client_uq,unique" json:"user_id"`
	User       *User             `gorm:"foreignKey:UserID" json:"user,omitempty"`
	BusinessID uuid.UUID         `gorm:"type:uuid;not null;index" json:"business_id"`
	Business   *BusinessVertical `gorm:"foreignKey:BusinessID;references:ID" json:"business,omitempty"`
	ClientKey  string            `gorm:"size:100;not null;default:default;index:idx_user_active_business_user_client_uq,unique" json:"client_key"`
